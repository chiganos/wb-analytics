...
def get_optimal_price(df, y_col):
    ...
    price_range = pd.DataFrame(np.linspace(...), columns=["price"])
...
def analyze_price_optimization(db_path: str, article: int) -> HTMLResponse:
    ...
    if df_orders is not None:
        ...
        fig.add_trace(go.Scatter(
            x=np.linspace(df_orders["price"].min(), df_orders["price"].max(), 200),
            y=y_orders,
            mode="lines",
            name=f"Оптимальная цена по заказам: {int(price_orders)}",
    ...
    if df_profit is not None:
        ...
        fig.add_trace(go.Scatter(
            x=np.linspace(df_profit["price"].min(), df_profit["price"].max(), 200),
            y=y_profit,
            mode="lines",
            name=f"Оптимальная цена по прибыли: {int(price_profit)}",
