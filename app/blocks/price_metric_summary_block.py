...
def compute_current_metrics(article_id: int, conn: sqlite3.Connection) -> dict | None:
    ...
    upload = pd.read_sql_query(
        "SELECT stock, cost_price, avg_buyout_percent, name FROM upload WHERE article = ?", conn,
        params=(article_id,)
    )
    wb_stock = int(funnel['stocks_wb'].iat[-1])
    up_stock = int(upload['stock'].iat[0]) if not upload.empty else 0
    total_stock = wb_stock + up_stock
    cost_price = float(upload['cost_price'].iat[0]) if not upload.empty else 0.0
    buyout_pct = float(upload['avg_buyout_percent'].iat[0]) if not upload.empty else 0.0
    name = upload['name'].iat[0] if not upload.empty else ''

    # рассчитываем совокупные показатели
    ...
    # показатели
    nominal_per_day = round(avg_profit * avg_sales, 1)
    rest_cost = round(total_stock * cost_price, 1)
    ...
    return {
        'article': article_id,
        'Название': f"<div style='font-size:0.8em;white-space:normal'>{name}</div>",
        'Дней до распродажи': days_to_clear,
        'Номинальная прибыль в день': nominal_per_day,
        'Себестоимость остатков': rest_cost,
...
def render_html_table(df: pd.DataFrame) -> str:
    ...
    <style>
      ...
      td div {font-size:0.8em; white-space:normal;}
    </style>
    """
    return styles + df.to_html(index=False, escape=False, float_format='{:.0f}'.format)
...
def summary_single(article: int):
    ...
    conn.close()
    if best_df.empty:
        raise HTTPException(status_code=404, detail="Недостаточно данных для диапазонов")
    best = best_df.loc[best_df['roi_net'].idxmax()]
    data = {**curr,
            'Лучший ROI чистая (%)': round(best['roi_net'], 1),
            'Лучшая годовая рентабельность (%)': round(best['annual_return'], 1)}
    df = pd.DataFrame([data])
    return HTMLResponse(render_html_table(df))
...
def summary_all():
    ...
    summary = {
        'article': 'Итого',
        'Название': '',
        'Дней до распродажи': round(df['Дней до распродажи'].mean(), 1),
        'Номинальная прибыль в день': round(df['Номинальная прибыль в день'].sum(), 1),
        'Себестоимость остатков': round(df['Себестоимость остатков'].sum(), 1),
        'ROI чистая (%)': round(df['ROI чистая (%)'].mean(), 1),
        'Чистая годовая рентабельность (%)': round(df['Чистая годовая рентабельность (%)'].mean(), 1),
        'Лучший ROI чистая (%)': round(df['Лучший ROI чистая (%)'].mean(), 1),
        'Лучшая годовая рентабельность (%)': round(df['Лучшая годовая рентабельность (%)'].mean(), 1)
    }
