...
def analyze_keywords(db_path: str, article: str):
    ...
    for _, row in result.iterrows():
        ...
        fig_kw = go.Figure()
        fig_kw.add_trace(go.Scatter(x=subset["date"], y=subset["open_card"], name="Переходы", mode="lines+markers"))
        fig_kw.add_trace(go.Scatter(x=subset["date"], y=subset["cart_add"], name="Добавления в корзину", mode="lines+markers"))

        fig_kw.update_layout(
            title=f"{keyword} — Корреляция: {corr}, Переходов: {total}",
            xaxis_title="Дата",
        ...
        html_blocks.append(f"""
            <details style='margin-bottom:1em;'>
              ...
              {fig_kw.to_html(full_html=False, include_plotlyjs=False)}
            </details>
        """)
